...
#define SIGN_EXTEND_IMM(x)   do { x <<= 16; x >>= 16; } while (0)
   ...
   case 0:
      ...
      while(inROB(i.reg.rs) || inROB(i.reg.rt)) PAUSE(1);
      //pushInROB(_decodedDST);
   ...
   case 4:			// beq
      ...
      while(inROB(i.imm.rs) || inROB(i.imm.rt)) PAUSE(1);
   ...
   if(_writeREG || _writeFREG) pushInROB(_decodedDST);
} 
...
void
Mipc::pushInROB(unsigned destReg){
   ROB.push_front(destReg);
}
...
bool
Mipc::inROB(unsigned srcReg){
//printf("SrcReg: %u Printing ROB Contents:\n", srcReg);
   for (std::list<unsigned>::iterator it = ROB.begin(); it != ROB.end(); it++){
      if(*it == srcReg){
         _dataPipeStalled = 1;
         _pipeReg_de[0] = 0;
         return true;
