         ...
         if (isSyscall) {
...
#endif      
            // _mc->_pipeStalled = TRUE;
            _mc->_opControl(_mc, ins);
            _mc->_pipeStalled = FALSE;
            ...
            if (writeReg) {
               ...
               if (loWPort) {
                  _mc->_lo = opResultLo;
#ifdef MIPC_DEBUG
                  fprintf(_mc->_debugLog, "<%llu> Writing to Lo, value: %u for ins %#x\n", SIM_TIME, opResultLo, ins);
#endif
               }
               if (hiWPort) {
                  _mc->_hi = opResultHi;
#ifdef MIPC_DEBUG
                  fprintf(_mc->_debugLog, "<%llu> Writing to Hi, value: %u for ins %#x\n", SIM_TIME, opResultHi, ins);
#endif
               }
            }
         }
